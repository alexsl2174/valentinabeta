...
class Admin(commands.Cog):
  ...
  @commands.hybrid_command()
  @commands.guild_only()
  async def revokeownership(self, ctx: commands.Context, user_id: str):
    """Revoke all relationships with the user from the database."""
    try:
      user_id = int(user_id)
      m = await self.bot.fetch_user(user_id)
    except discord.NotFound:
      await ctx.send(f"<:no:1178686922768519280> User with id {user_id} not found.")
      return
    except ValueError:
      await ctx.send(f"<:no:1178686922768519280> user_id must be an integer.")
      return

    database.remove_member(user_id, ctx.guild.id)
    await ctx.send(f"<:yes:1184312448912732180> All relationships with {m} ({m.id}) are now gone.")
