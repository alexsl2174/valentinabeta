...
def disown_a_slave(slave, owner, guild):
  with con:
    cur.execute("DELETE FROM Ownership WHERE slaveid = %s AND guildid = %s AND ownerid = %s", (slave, guild, owner))
    cur.execute("DELETE FROM Badwords WHERE slaveid = %s AND guildid = %s", (slave, guild))
    cur.execute(
      "UPDATE SlaveDB SET gag=%s, tiechannel=%s, emoji=%s, chastity=%s, muff=%s WHERE slaveid=%s AND guildid = %s",
      ['off', 0, True, True, True, slave, guild])
